...
test.describe('Interactive Webpage with 2 Buttons Tests', () => {
  test('TC 1: should display initial message state', async ({ page }) => {
    // Navigate to the local HTML file
    const filePath = path.join(__dirname, '../js-example/02 Interactive Webpage/interactive2Buttons.html');
    ...
    // Check if the initial font color is #333 (rgb(51, 51, 51))
  ...
  test('TC 2: should change message and color after changeButton clicked', async ({ page }) => {
    // Navigate to the local HTML file
    const filePath = path.join(__dirname, '../js-example/02 Interactive Webpage/interactive2Buttons.html');
    await page.goto(`file://${filePath}`);
  ...
  test('TC 3: should reset message and color after resetButton clicked', async ({ page }) => {
    // Navigate to the local HTML file
    const filePath = path.join(__dirname, '../js-example/02 Interactive Webpage/interactive2Buttons.html');
    await page.goto(`file://${filePath}`);
    ...
    // First, click the change button to change the message
    await changeButton.click();
    ...
    // Add delay to see the reset action
    await page.waitForTimeout(2000);

    // Check if the text is reset back to the original message
    await expect(messageElement).toHaveText('Click the button to change this message!');

    // Check if the font color is reset back to #333 (rgb(51, 51, 51))
    const color = await messageElement.evaluate((el) => {
      return window.getComputedStyle(el).color;
    });
    expect(color).toBe('rgb(51, 51, 51)');
